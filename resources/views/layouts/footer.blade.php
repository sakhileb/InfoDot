<footer class="block pb-4">
    <hr class="mb-4 border-b-1 border-gray-300">
    <div class="flex flex-wrap md:justify-between justify-center">
        <div class="w-full md:w-5/12 px-4 flex flex-wrap justify-center sm:justify-start sm:ml-32">
            <div class="text-sm text-gray-600 font-semibold py-1">
                Copyright © <span id="javascript-date"></span>
                <a href="https://www.infodot.co.za" class="text-gray-600 hover:text-gray-800 text-sm font-semibold py-1">InfoDot</a>
            </div>
        </div>
        <div class="w-full md:w-5/12 px-4 flex flex-wrap justify-center">
            <ul class="flex flex-wrap list-none md:justify-end justify-center">
                <li>
                    <a href="https://www.infodot.co.za" class="text-gray-700 hover:text-gray-900 text-sm font-semibold block py-1 px-3">InfoDot</a>
                </li>
                <li>
                    <a href="{{ route('about') }}" class="text-gray-700 hover:text-gray-900 text-sm font-semibold block py-1 px-3">About Us</a>
                </li>
                <li>
                    <a href="{{ route('contact') }}" class="text-gray-700 hover:text-gray-900 text-sm font-semibold block py-1 px-3">Contact Us</a>
                </li>
                <li>
                    <a href="{{ route('terms') }}" class="text-gray-700 hover:text-gray-900 text-sm font-semibold block py-1 px-3">Terms & Conditions</a>
                </li>
            </ul>
        </div>
    </div>
</footer>
